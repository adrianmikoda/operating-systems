...
static int left_frames_gen = 0;
static int right_frames_gen = 0;
static int pairs_gen = 0;
static int robot_states_gen = 0;
static pthread_mutex_t mutex_stats = PTHREAD_MUTEX_INITIALIZER;

static void inc_left_frames(void) {
    pthread_mutex_lock(&mutex_stats);
    left_frames_gen++;
    pthread_mutex_unlock(&mutex_stats);
}

static void inc_right_frames(void) {
    pthread_mutex_lock(&mutex_stats);
    right_frames_gen++;
    pthread_mutex_unlock(&mutex_stats);
}

static void inc_pairs(void) {
    pthread_mutex_lock(&mutex_stats);
    pairs_gen++;
    pthread_mutex_unlock(&mutex_stats);
}

static void inc_robot_states(void) {
    pthread_mutex_lock(&mutex_stats);
    robot_states_gen++;
    pthread_mutex_unlock(&mutex_stats);
}

static void get_stats(int *l, int *r, int *p, int *s) {
    pthread_mutex_lock(&mutex_stats);
    *l = left_frames_gen;
    *r = right_frames_gen;
    *p = pairs_gen;
...
static void push_pair(stereo_pair_buffer_t *buf, const stereo_pair_t *pair, bool *run_flag) {
...
static bool pop_pair(stereo_pair_buffer_t *buf, stereo_pair_t *pair, bool *run_flag) {
    bool success = false;
    pthread_mutex_lock(&buf->mutex);
    while (buf->count == 0 && *run_flag) {
        pthread_cond_wait(&buf->cond_not_empty, &buf->mutex);
    }
    if (*run_flag && buf->count > 0) {
...
void *sync_thread_func(void *arg) {
    ...
    while (running) {
        ...
        if (diff < SYNC_THRESHOLD_MS) {
            ...
            push_pair(&pair_buffer, &pair, (bool*)&running);
            inc_pairs();
...
void *image_writer_thread_func(void *arg) {
    ...
    while (running) {
        stereo_pair_t pair;
        if (!pop_pair(&pair_buffer, &pair, (bool*)&running)) {
            break;
        }
...
int main(void) {
    ...
    int elapsed = 0;
    while (running && elapsed < RUN_DURATION_SEC) {
        sleep(5);
        elapsed += 5;
        if (running) {
            int l, r, p, s;
